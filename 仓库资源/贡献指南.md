...
状态条 `"This branch is even tiwh Xovee:master"` 代表您 Fork 的仓库与源仓库是完全相同的。|![even with origin](./img/pull-request-2.png)
然后就可以对您 Fork 的仓库做各种改动，例如修改 README.md 文件；修改完成后，点击 ![commit changes button](./img/pull-request-3.png) 按钮提交 `Commit`。|![edit readme.md](./img/pull-request-4.gif)
您也可以上传文件或者文件夹，进入您想要上传资源的目录，点击右上角的 `Upload files` 按钮，选择或者拖动文件/文件夹到网页中间的框内，然后提交 `Commit`。|![upload files or directory](./img/pull-request-5.gif)
在完成所有改动之后，回到仓库首页，您可以看到状态条显示当前分支比源仓库领先 `N` 个提交。|![ahead](./img/pull-request-6.png)
此时点击状态条右侧的 `Pull Request` 按钮或者 `Compare` 按钮![pull request or compare button](./img/pull-request-7.png)，即可查看您所做出的改动，然后点击 ![create pull request button](./img/pull-request-8.png) 按钮。|![create pull request](./img/pull-request-9.gif)
