**首先感谢各位有意为仓库做出贡献的贡献者们，仓库之建设苟非一日能成，亦非一二人为之；所谓“九层之台，起于垒土；千里之行，始于足下”，仓库资源若能积少成多、由简至臻，全凭各位贡献者的无私劳动，没有你们，便不可能有仓库的未来。❤️❤️❤️**

太长不读版：如果您不熟悉Github，可以直接将资源发送至我的邮箱`xovee at uestc.edu.cn`；如果您熟悉Github，可以使用Pull Request贡献资源。
...
除了为仓库贡献或改善资源，您还可以为仓库提出改进意见，包括但不限于仓库的各种组织架构、设计细节等问题。您可以通过提交 Issue，提交 Pull Request，或者通过邮件（xovee at uestc.edu.cn）来提出改进意见。
...
3. 使用 Email 或者 Issue 把资源和表格发送给我们（建议压缩为 ZIP 文件）。注意，请在 Email 或者 Issue 中提供必要的课程或资源信息（例如课程全名、考试日期和方式等）。

Email 地址：xovee at uestc.edu.cn
...
`"This branch is even with Xovee:main"` <br>代表您 Fork 的仓库与源仓库是完全相同的。|![even with origin](./img/pull-request-2.png)
然后就可以对您 Fork 的仓库做各种改动，<br>例如修改 README.md 文件；修改完成后，<br>点击 <img src='./img/pull-request-3.png' height=25px> 按钮提交 `Commit`。<br> |![edit readme.md](./img/pull-request-4.gif)
...
假如您对 Fork 的仓库提交了两次 `Commit`，<br>`This branch is 2 commits ahead of Xovee:main`<br>表示当前分支比源仓库领先 `2` 个提交。|![ahead](./img/pull-request-6.png)
...
如果您上传的图片文件很大，建议使用 https://squoosh.app/ 服务来将图片转换为webp格式，文件大小一般可以减少50-80%。
